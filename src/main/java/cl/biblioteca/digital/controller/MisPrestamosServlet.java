...
public class MisPrestamosServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("ADMIN".equals(usuario.getRol())) {
            lista = prestamoDAO.listarTodosActivos();
        } else {
            lista = prestamoDAO.listarPorUsuario(usuario.getIdUsuario());
        }

        request.setAttribute("prestamos", lista);
        request.getRequestDispatcher("/WEB-INF/view/mis-prestamos.jsp").forward(request, response);
    }
}